        ...
        >
          ...
          <el-form-item prop="verifycode" label="验证码：">
            ...
            <el-input
              ...
              placeholder="请输入验证码"
              class="identifyinput nnm"
              @keyup.enter.native="handLogin"
            ></el-input>
            <div class="identifybox">
              <div @click="refreshCode" class="login1"></div>
            </div>
          </el-form-item>
          <el-form-item class="check">
            ...
            <span class="howName" style="cursor: pointer;">
              <el-popover placement="bottom" width="200" popper-class="bu" trigger="click">
                <div>
                  <span class="xuan1">&emsp;</span>
                  <span
                    class="xuan"
                  >请您先点击【注册】进行供应商信息的录入，注册完成后，请耐心等待。你的资料将在3-5个工作日内完成审核，届时请保持联系人手机畅通，方便我司工作人员与您联系并告知登录账号。</span>
                </div>
                <span class="howNamered el-icon-question" slot="reference">
                  <b
                    style="font-size:14px;font-family:MicrosoftYaHei;font-weight:400;color:rgba(153,153,153,1);"
                  >如何获取账户信息</b>
                </span>
              </el-popover>
            </span>
          ...
          <el-form-item class="loginButto-item">
            ...
            >登录</el-button>
            <el-button type="primary" @click.native.prevent="handleReg">注册</el-button>
...
export default {
  ...
  methods: {
    ...
    getAjax() {
      ...
      axios
        .post(this.$apiUrl.api + "/supLogin.jsp", {
    ...
    initDom() {
      if (980 < $(document.body.clientWidth).height() < 1300) {
        console.log(this.cavesW), (this.cavesW = 50);
      }
    },
    randomNum(min, max) {
    ...
    handLogin() {
      if (this.loginForm.name == "") this.$message.error("请输入用户名");
      else if (this.loginForm.password == "") this.$message.error("请输入密码");
        ...
        (this.loginIF && this.loginIF_1 && this.loginIF_2)
      ) {
        this.loading = true;
        $(".login1").click();
        this.getAjax();
